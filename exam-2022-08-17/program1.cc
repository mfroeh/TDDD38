...
public:
  ...
  template <typename... Ts> Counted_Pointer(Ts &&...args);
...
template <typename T> Counted_Pointer<T>::Counted_Pointer() : block{nullptr} {}
...
template <typename T>
Counted_Pointer<T>::Counted_Pointer(Counted_Pointer<T> const &other)
    : block{other.block} {
  if (block) {
    block->count++;
  }
...
template <typename T>
Counted_Pointer<T>::Counted_Pointer(Counted_Pointer<T> &&other)
    : block{other.block} {
  other.block = nullptr;
}
...
template <typename T> Counted_Pointer<T>::~Counted_Pointer() {
...
template <typename T> T &Counted_Pointer<T>::operator*() { return block->data; }

template <typename T> T const &Counted_Pointer<T>::operator*() const {
  return block->data;
}

template <typename T> T *Counted_Pointer<T>::operator->() {
  return &(block->data);
}
...
template <typename T> int Counted_Pointer<T>::count() const {

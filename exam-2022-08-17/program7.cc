...
template <typename T>
requires(!Iter<T>) size_t count_bytes(T t) { return sizeof(t); }

template <Iter T> size_t count_bytes(T const &obj) {
  size_t sum{};
  for (auto &&e : obj) {
    sum += count_bytes(e);
  }
...
template <typename T, size_t N> size_t count_bytes(T (&arr)[N]) {
  size_t sum{};
  for (auto &&e : arr) {
    sum += count_bytes(e);
  }

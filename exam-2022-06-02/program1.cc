...
Sentinel make_list() { return {}; }

template <typename T, typename... Args>
auto make_list(T &&first, Args &&...rest) {
  return std::make_pair(std::forward<T>(first),
                        make_list(std::forward<Args>(rest)...));
}

template <size_t N, typename T> struct Get_Index {
  static auto &value(T &list) {
    return Get_Index<N - 1, typename T::second_type>::value(list.second);
  }
};

template <typename T> struct Get_Index<0, T> {
  static auto &value(T &list) { return list.first; }
};

template <size_t N, typename T> auto &get_index(T &list) {
